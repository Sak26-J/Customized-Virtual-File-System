...
int rm_File(char *name) {
    ...
    if (UFDTArr[fd].ptrfiletable->ptrinode->LinkCount == 0) {
        ...
        free(UFDTArr[fd].ptrfiletable->ptrinode->Buffer); // Free buffer memory
...
int LseekFile(int fd, int size, int from) {
    ...
    if ((UFDTArr[fd].ptrfiletable->mode == READ) || (UFDTArr[fd].ptrfiletable->mode == READ + WRITE)) {
        if (from == CURRENT) {
            ...
            if ((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size > MAXFILESIZE || ((UFDTArr[fd].ptrfiletable->readoffset) + size) < 0)
                return -1;
        ...
        if (from == CURRENT) {
            ...
            if ((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size > MAXFILESIZE || ((UFDTArr[fd].ptrfiletable->writeoffset) + size) < 0)
                return -1;
    ...
    while (1) {
        ...
        if (count == 1) {
            if (strcmp(command[0], "ls") == 0) {
                ...
                system("clear"); // On Unix-like systems, use "clear" instead of "cls"
                continue;
